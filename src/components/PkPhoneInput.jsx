...
// A paste button + a clipboard hint let you drop in a number you copied from the
// phone's call log without retyping it.
    ...
    <div className="pk-phone-field">
      ...
      {hint && !value && (
        <button type="button" className="pk-phone-hint" onClick={useHint}>
          Use <b>{hint.slice(0, 3)} {hint.slice(3)}</b> from clipboard
        </button>
      )}
    </div>
  )
